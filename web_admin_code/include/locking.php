...
class oLocking
{
	...
	function oLocking()
	{
		...
		$this->ConfirmAdmin = "%S 管理员中止您编辑会话 当管理员将中止您编辑会话锁定工具栏会出现的消息";
		$this->ConfirmUser = "您的编辑会话超时 '记录锁定' 工具栏上将显示的消息";
		$this->LockAdmin = "记录是由 %s%s 分钟内进行编辑  记录锁定消息： ' 记录由 admin 编辑 12 分钟";
		$this->LockUser = "记录是由另一个用户编辑  '记录锁定' 工具栏上将显示的消息";
		...
		if( $data = $qResult->fetchAssoc() )
		{
			...
			if($links){
				$str.='<a class="unlock" href="#" onclick="Runner.pages.PageManager.getAt(\''.runner_htmlspecialchars(jsreplace($strtable)).'\', '.$pageid.').locking.UnlockAdmin(\''
					.runner_htmlspecialchars(jsreplace($skeys)).'\',\''.$data["sessionid"].'\',\'no\');return false;">'."解除锁定记录 '解锁记录' 的消息。适用于记录锁定。".'</a>';
				$str.='<a class="edit" href="#" onclick="Runner.pages.PageManager.getAt(\''.runner_htmlspecialchars(jsreplace($strtable)).'\', '.$pageid.').locking.UnlockAdmin(\''
					.runner_htmlspecialchars(jsreplace($skeys)).'\',\''.$data["sessionid"].'\',\'yes\');return false;">'."编辑记录 编辑记录的消息。将显示记录锁定工具栏上的管理员".'</a>';
			}
			return $str;
		}
