...
class UploadHandler
{
    ...
    public function validate($uploadedFile, &$file, $error, $file_size, $index, $uploadDir) {
        ...
        if ($this->options['max_file_size'] && (
            ...
            $file["error"] = mysprintf("文件大小超出限制的 %s kb 为单位）", array($this->options['max_file_size']));
            return false;
        }
        if ($this->options['min_file_size'] &&
            $file_size < $this->options['min_file_size'] * 1024) {
            $file["error"] = mysprintf("文件大小不能小于 %s kb 为单位）", array($this->options['min_file_size']));
            return false;
        }
   		if (is_int($this->options['max_totalFile_size']) && (
                $this->getUploadFilesSize() + $file["size"] > $this->options['max_totalFile_size'] * 1024)
            ) {
            $file["error"] = mysprintf("总的文件大小超过了限制的 %s kb 为单位）", array($this->options['max_totalFile_size']));
            return false;
        }
        if (is_int($this->options['max_number_of_files']) && (
            	...
            	if($this->options['max_number_of_files'] > 1)
            		$file["error"] = mysprintf("你可以上传文件 %s 不超过", array($this->options['max_number_of_files']));
            	else 
            		$file["error"] = "您可以将只有一个文件上载";
            return false;
        }
